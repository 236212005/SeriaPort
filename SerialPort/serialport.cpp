...
char* SerialPort::ReadSerialPort() {
	...
	if (ReadFile(hCom, &serialContent, sizeof(serialContent), &wCount, NULL))
	{
		...
		for (int count = 0; count <= 1000; count++) {
			if (serialContent[count] == 2) {
				index = count;
				break;
			}
		};
		if (serialContent[index + 10] == 32 && serialContent[index + 3] != 13) {
			
			temp[1] = serialContent[index + 5];
			temp[2] = serialContent[index + 6];
			temp[3] = serialContent[index + 7];
			temp[4] = serialContent[index + 8];
			temp[5] = serialContent[index + 9];
			temp[6] = serialContent[index + 10];
			temp[7] = '\0';
			/*string ok = temp;
			if (缓冲[index + 5] == 32) {
				cout << "不稳定:" << ok << endl;
